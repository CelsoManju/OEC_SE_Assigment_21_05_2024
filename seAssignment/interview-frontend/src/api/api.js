...
export const addUsersToPlanProcedure = async (planId, procedureId,userIds) => {
    const url = `${api_url}/PlanProcedure/AddUsersToPlanProcedure`;
    ...
    if (!response.ok) throw new Error("Error: Failed to add users to plan procedure list");

    return true;
};

export const getPlanProcedureUsers = async (planId, procedureId) => {
    const url = `${api_url}/PlanProcedure/${planId}/${procedureId}/PlanProcedureUsers`;
    const response = await fetch(url, {
        method: "GET",
        headers: {
            Accept: "application/json",
    ...
    if (!response.ok) throw new Error("Error: Failed to fetch assigned users of plan procedure");
