...
const Plan = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      for (const planProcedure of planProcedures) {
        const usersForProcedure = await getPlanProcedureUsers(id, planProcedure.procedureId);
        assignedUsersData[planProcedure.procedureId] = usersForProcedure.map(userId => userOptions.find(u => u.value === userId));
      }
  ...
  const handleAddUserToProcedure = async (planId, procedureId, userIds) => {

    try {
        await addUsersToPlanProcedure(planId, procedureId, userIds);
                    ...
                    <div>
                      {planProcedures.map((p) => (
                        <PlanProcedureItem
                          ...
                          handleAddUserToProcedure ={handleAddUserToProcedure}
                        />
                      ))}
                    </div>
                  </div>
